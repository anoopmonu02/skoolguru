 ...
 * mechanics (per-row instant upload vs. Save-button batching) are a
 * separate concern and can be added as its own group later if wanted.
 */
public class StudentFieldGroupRegistry {
    ...
    static {
        ...
        GROUPS.add(new GroupDef("GENDER", "Gender", List.of(
                new FieldDef("gender", "Gender", Widget.SELECT)
        )));
        GROUPS.add(new GroupDef("QUALIFICATION", "Father Qualification, Mother Qualification", List.of(
        ...
        GROUPS.add(new GroupDef("HEIGHT_WEIGHT", "Height(cm), Weight(kg)", List.of(
                new FieldDef("height", "Height(cm)", Widget.NUMBER),
                new FieldDef("weight", "Weight(kg)", Widget.NUMBER)
        )));
        GROUPS.add(new GroupDef("HEALTH_EYE_ISSUE",
                "Body Type, Have Health Issue, Health Issue Description, Have Eye Issue", List.of(
