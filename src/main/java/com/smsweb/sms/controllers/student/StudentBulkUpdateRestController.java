 ...
 *
 * Role gate matches StudentBulkUpdateController (the page) exactly, so a role
 * that can see the page can also actually call these endpoints.
 */
@RestController
...
public class StudentBulkUpdateRestController extends BaseController {
    ...
    public ResponseEntity<?> getStudentsByFieldGroup(@RequestBody Map<String, String> requestBody, Model model) {
        ...
        try {
            ...
            // never from the request body — the same IDOR-safe pattern the existing
            // getStudentsForSR endpoint already uses.
