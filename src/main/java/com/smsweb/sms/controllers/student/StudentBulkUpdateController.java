 ...
 *
 * Role gate matches the sidebar's "Students" menu (Admin / Super Admin /
 * Teacher) so the page and the REST endpoints behind it
 * (StudentBulkUpdateRestController) never disagree about who can use this
 * screen — the fine-grained STUDENT_UPDATE_DETAILS permission (below) still
 * governs per-user access within that role set, same as every other
 * @CheckAccess-gated Student screen.
 */
@Controller
